...
/** Thrown when `fetch` itself fails (no connectivity, DNS failure, etc.) rather than the server responding with an error — lets callers fall back to cached data. */
...
export async function validateStoredSession(): Promise<boolean> {
  ...
  try {
    ...
    if (err instanceof NetworkError) {
      // Offline at startup with a previously-stored session: let the app in
      // rather than bouncing to login — screens fall back to cached data.
      return true;
    }
    throw err;
  }
...
/**
 * Authenticated fetch for the field staff app: attaches the stored access
