...
/**
 * Authenticated fetch for the tenant app: attaches the stored access token,
 * and on a 401 transparently refreshes once and retries — mirrors the
 * silent-rotation the web app's proxy.ts does server-side, except here it
 * happens client-side since there's no BFF in front of a mobile app.
 */
